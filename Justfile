...
# Recipe to run the reva CLI for a particular block and chain id.
run-block block_number chain_id:
    cargo run --release --bin reva -- --block-number {{block_number}} --chain-id {{chain_id}}
...
# Recipe to run the reva CLI for a range of blocks.
...
# Recipe to run the reva CLI (with tracing) for a block and chain id.
trace-block block chain_id:
    TRACE_FILE=trace_$block_$chain_id.log cargo run --release --bin reva -- --block-number "$block_number" --chain-id {{chain_id}}
    cargo prove --trace 
