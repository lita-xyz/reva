...
use reva_host_executor::{EthHostExecutor, HostExecutor};
use reva_primitives::genesis::Genesis;
...
async fn test_e2e_optimism() {
    ...
    let host_executor = reva_host_executor::OpHostExecutor::optimism(chain_spec.clone());

    // Setup the client executor.
    let client_executor = reva_client_executor::executor::OpClientExecutor::optimism(chain_spec);
